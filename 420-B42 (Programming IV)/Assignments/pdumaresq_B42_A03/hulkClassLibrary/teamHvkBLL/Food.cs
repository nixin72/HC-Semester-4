...
namespace teamHvkBLL {
    public class Food {
        ...
        public List<Food> ListFoods() {
            ...
            DataTable data = food.GetFullFoodInfo();
            return GetFoods(data);
        }

        private List<Food> GetFoods( DataTable data ) {
            List<Food> list = new List<Food>();
            foreach ( DataRow r in data.Rows ) {
                try {
                    list.Add(new Food(
                        r.IsNull("FOOD_NUMBER") ? (int)r["FOOD_NUMBER"] : -1,
                        r.IsNull("FOOD_BRAND") ? (string)r["FOOD_BRAND"] : "",
                        r.IsNull("FOOD_VARIETY") ? (string)r["FOOD_VARIETY"] : ""
                    ));
                } catch { }
            };
